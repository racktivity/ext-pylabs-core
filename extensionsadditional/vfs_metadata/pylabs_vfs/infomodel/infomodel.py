...
class FileNode(object):
    isLeaf = True
    def __init__(self, name, size, moddate, crtdate, md5hash="", dataKey='', docId=''):
        self.name = name
        self.size = size
    ...
    def __getitem__(self, idx):
        ...
        return (self.name, self.size, self.moddate, self.crtdate, self.md5hash, self.dataKey, self.docId)[idx]
...
class DirNode(object):
    ...
    def serialize(self):
        ...
        for key in sorted(self.files):
            name, size, moddate, crtdate, md5hash, dataKey, docId = self.files[key]
            content="%sF:%s|%s|%s|%s|%s|%s|%s\n"%(content, name, size, moddate, crtdate, md5hash, dataKey, docId)
    ...
    def deserialize(self, key, content):
        ...
        for line in lines2:
            if line.strip():
                if line[0:2]=="F:":
                    [name, size, modDate, crtdate, md5hash, dataKey, docId] = line[2:].split("|")
                    self.addFile(name,long(size),float(modDate), float(crtdate) ,md5hash, dataKey, docId)
                if line[0:2]=="D:":
    ...
    def addFile(self, name, size=0, moddate=0, crtdate=0, md5hash='', dataKey='', docId= ''):
        self.files[name.strip()] = FileNode(name, size, moddate, crtdate, md5hash, dataKey, docId)
    ...
    def addFilePath(self, fileFullPath):
        ...
        if self.usemd5:
            ...
            md5hash = ""            
        self.addFile(filename, size, moddate, crtdate, md5hash, docId)
