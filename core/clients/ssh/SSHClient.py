...
class SSHClient:
    ...
    def executewait(self, command,dieOnError=True):
        ...
        q.logger.log("Execute ssh command %s on %s" % (command,self.host))
    ...
    def getSFtpConnection(self):
        q.logger.log("Open SFTP connection to %s" % (self.host))
