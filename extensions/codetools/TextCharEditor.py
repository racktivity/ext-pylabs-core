...
class TextCharEditor():
    ...
    def getText(self):
        return string.join([char[0] for char in self.chars],"")
    ...
    def matchBlocksPattern(self,startpattern,stoppattern,blockname):
        ...
        """
        result=q.codetools.regex.getRegexMatches(startpattern,self.getText())
        if len(result.matches)==0:
            pass
        for match in result.matches:    
            start=match.start
            textToInvestigate=string.join([char[0] for char in self.chars[match.start:]],"")
            result2=q.codetools.regex.getRegexMatches(stoppattern,textToInvestigate)     
            ...
            for pos in range(match.start,match.start+end): #scan first time if somewhere there is already a char part of a block
                if self.chars[pos][1]<>"":
                    skip=True
                    #q.logger.log("Could not match the pattern because as part of result there was already another block found, posstart:%s posstop%s" % (match.start,match.start+end-1),5)
