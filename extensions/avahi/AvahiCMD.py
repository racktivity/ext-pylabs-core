...
class AvahiCMD():
    ...
    def getServices(self):
        ...
        result,output=q.system.process.execute(cmd,False,False)
        if result>0:
            raise RuntimeError("cannot use avahi command line to find services, please check avahi is installed on system (ubunutu apt-get install avahi-utils)\nCmd Used:%s" % cmd)
        items=q.codetools.regex.extractBlocks(output,["^= .*"])
        ...
        for item in items:
            ...
            s.hostname=q.codetools.regex.getINIAlikeVariableFromText(" *hostname *",item).replace("[","").replace("]","").strip()
            s.address=q.codetools.regex.getINIAlikeVariableFromText(" *address *",item).replace("[","").replace("]","").strip()
            s.port=q.codetools.regex.getINIAlikeVariableFromText(" *port *",item).replace("[","").replace("]","").strip()
            s.txt=q.codetools.regex.getINIAlikeVariableFromText(" *txt *",item).replace("[","").replace("]","").strip()
    ...
    def resolveAddress(self, ipAddress):
        ...
        """
        #do some validation
        if not q.system.net.validateIpAddress(ipAddress):
            raise ValueError('Invalid Ip Address')
        cmd = 'avahi-resolve-address %s'
        exitCode, output = q.system.process.execute(cmd%ipAddress, dieOnNonZeroExitCode = False, outputToStdout = False)
