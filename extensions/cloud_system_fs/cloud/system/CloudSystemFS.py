...
class CloudSystemFS:
    ...
    def fileGetContents(self, url):
        tempfile = q.system.fs.getTmpFilePath()
        self.copyFile(url, 'file://' + tempfile)
        content = q.system.fs.fileGetContents(tempfile)
        q.system.fs.removeFile(tempfile)
        return content

    def writeFile(self, url, content):
        tempfile = q.system.fs.getTmpFilePath()
        q.system.fs.writeFile(tempfile, content)
        self.copyFile('file://' + tempfile, url)
        q.system.fs.removeFile(tempfile)

    """
    set of library actions which are used in cloud
    these are different from q.system.fs in the fact they support more generic url's and are specific to moving data around on a cloud/network level
    extension which is linked onto q.cloud.system.fs.
    """
    def copyFile(self,sourcepath,destinationpath,tempdir=q.dirs.tmpDir):
        """
        ...
        # tested scenarios
        # 
        # q.cloud.system.fs.copyFile('ftp://ftp.belnet.be/mirror/ftp.centos.org/HEADER.html',  'cifs://aserver:aserver@localhost/share/testje')
        # q.cloud.system.fs.copyFile('file:///tmp/src', 'cifs://aserver:aserver@localhost/share')
        # q.cloud.system.fs.copyFile('file:///tmp/src', 'ftp://localhost/anon/remote-ftp')
        # q.cloud.system.fs.copyFile('ftp://ftp.belnet.be/mirror/ftp.centos.org/HEADER.html', 'ftp://localhost/anon/remote-ftp')
        # q.cloud.system.fs.copyFile('file:///mnt/blubje' , 'sftp://aserver:aserver@localhost/home/aserver/Desktop/')
        # q.cloud.system.fs.copyFile('sftp://aserver:aserver@localhost/home/aserver/Desktop/bub', 'file:///mnt/blubje')
        # Determine the object we need to call
        q.logger.log("copyFile: from [%s] to [%s]" % ( sourcepath, destinationpath) )
    ...
    def moveFile(self,sourcepath, destinationpath, tempdir=q.dirs.tmpDir):
        """
        Move a file
        """
        q.logger.log("moveFile: from [%s] to [%s]" % ( sourcepath, destinationpath) )
    ...
    def copyDir(self,sourcepath, destinationpath, tempdir=q.dirs.tmpDir, recursive=True):
        """
        Copy Directory
        """
        q.logger.log("copyDir: from [%s] to [%s]" % ( sourcepath, destinationpath) )
    ...
    def moveDir(self,sourcepath, destinationpath, tempdir=q.dirs.tmpDir, recursive=True):
        """
        Move directory
        """
        q.logger.log("moveDir: from [%s] to [%s]" % ( sourcepath, destinationpath) )
    ...
    def _getSourceHandler(self, sourcepath, is_dir=False,recursive=True, tempdir=q.dirs.tmpDir, Atype='copy'):
        """
        ...
        q.logger.log('PARSING SRC RETURNED %s' %str(src_elements))

        # Determine the object we need to call
        q.logger.log("_getSourceHandler: source protocol [%s]" % (src_proto))
    ...
    def _getDestinationHandler(self,destinationpath,is_dir=False,recursive=True, tempdir=q.dirs.tmpDir, Atype='copy'):
        """
        Handle all protocol related stuff
        Returns a dict with the src and dst
        """
        ...
        q.logger.log('PARSING DEST RETURNED %s' %str(dst_elements))
        
        # Determine the object we need to call
        q.logger.log("_getDestinationHandler: destination protocol [%s]" % (dst_proto))
    ...
    def _determineProtocol(self, url):
        ...
        q.logger.log('Determined protocol: %s' %str(elements.scheme))
    ...
    def _parseCifsURL(self,url):
        ...
        ret_elements = {}
        q.logger.log('_parseCifsURL returned %s' %elements.hostname)
        return elements

    def importFile(self,sourcepath,destinationpath,tempdir=q.dirs.tmpDir):
        """
    ...
    def exportDir(self,sourcepath,destinationpath,recursive=True,tempdir=q.dirs.tmpDir):
        """
        ...
        # Tested: q.cloud.system.fs.importDir('smb://autotest:phun5chU@fileserver.aserver.com/Public/Engineering/vdi/sso_images/SSO_VD', 'file:///tmp/tmp/')
        q.logger.log("exportDir: from [%s] to [%s]" % (sourcepath, destinationpath) )
    ...
    def importDir(self,sourcepath,destinationpath,tempdir=q.dirs.tmpDir):
        """
    ...
    def exportVolume(self,sourcepath,destinationpath,format="vdi",tempdir=q.dirs.tmpDir):
        """
        ...
        if not tempdir and self._determineProtocol(destinationpath) in ("cifs", "smb"):
            ...
            if not q.system.fs.exists(directory):
                q.system.fs.createDir(directory)
            tmp_outputFileName = '%s/%s'%(directory,destination.filename)
            copy = False
        else:
            tmp_outputFileName = q.system.fs.getTempFileName(tempdir)

        q.logger.log("CloudSystemFS: exportVolume source [%s] to path [%s]" % (sourcepath, tmp_outputFileName))
        ...
        q.cmdtools.disktools.qemu_img.convert(sourcepath, 'raw', tmp_outputFileName, format, compressTargetImage=compressImage)
        outputFileName = ''.join([prefix,tmp_outputFileName])

        if copy:
            self.copyFile(outputFileName,destinationpath,tempdir=tempdir)
            q.system.fs.removeFile(tmp_outputFileName)
        else:
            destination.cleanup()

    def importVolume(self, sourcepath, destinationpath,format='vdi',tempdir=q.dirs.tmpDir):
        """
        ...
        """
        prefix = 'file://'
        q.logger.log("CloudSystemFS: importVolume source [%s] to path [%s]" % (sourcepath, destinationpath))
        ...
        if  protocol == "file":
            ...
            q.logger.log("Source is a local file:// not running copyFile... for %s" % elements.path)
            ...
            q.logger.log("Source is a CIFS/SMB share, not running copyFile,using %s" %tmp_inputFileName)
        else:
            tmp_inputFileName = q.system.fs.getTempFileName(tempdir)
            self.copyFile(sourcepath, ''.join([prefix,tmp_inputFileName]),tempdir=tempdir)

        q.cmdtools.disktools.qemu_img.convert(tmp_inputFileName, format, destinationpath, 'raw', compressTargetImage=compressImage)

        if not protocol == "file" and not protocol == "smb" and not protocol == "cifs":
            q.system.fs.removeFile(tmp_inputFileName)
        elif protocol == "smb" or protocol == "cifs":
            src_fs.cleanup()
    ...
    def listDir(self, path):
        ...
        q.logger.log("listDir: supplied path is [%s]" % path )

        proto = self._determineProtocol(path)
        if(proto == "cifs" or proto == "smb"):
            path_elements = self._parseCifsURL(path)
            q.logger.log('CIFS LISTDIR path_elements: %s' %str(path_elements))
        else:
            path_elements = urlparse.urlparse(path)

        # Determine the object we need to call
        q.logger.log("listDir: protocol [%s]" % proto )
