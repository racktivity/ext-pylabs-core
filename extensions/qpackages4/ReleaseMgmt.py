...
def compareDependencyFiles(file1, file2):
    """
    Compare two files with lists of qpackages, to find out which ones are new, which ones have been removed, and which ones have been changed between the two.
...
class ReleaseMgmt():
    """
    @qlocation = q.qp.releasemgmt
    """


    def generateBillOfMaterials(self, qpackageObject, outputFile):
        """
        Create a file with all qpackages and their versions that a given qpackage depends upon ('parents')
        The file will contain the domain, the name, the version and the build nr from the parent qpackages.
        The file will be sorted.

        Parameters: 
        - qpackageObject: Object - the qpackage object where you want to list the parents for
        - outputFile: string - the file where the parent qpackages list will be written to

        Returns: None
        """
        # sortPackage and sortLevelPackages are some auxiliary functions to sort qpackages
        ...
        with open(outputFile, 'w') as f:  
            parentpckgs = qpackageObject.getDependencies(recursive=True)
    ...
    def _listChangedPackagesAsObjects(self, earlierFile, laterFile, comparison='+'):
        ...
        for str in strlist:
            ...
            pckfoundlist = q.qp.find(name=pckattributes[1],domain=pckattributes[0], version=pckattributes[2])
    ...
    def createTgzForChangedPackages(self, earlierFile, laterFile, tempDir = '/opt/qbase3/var/tmp/changedpackages', tarName = 'changedPack.tgz'):
        ...
        Note that the packages and metadata are downloaded from the location specified in bundledownload in /opt/qbase3/cfg/qpackages4/sources.cfg
        ...
        if not q.system.fs.exists(q.system.fs.getParent(absTempDir)):
            raise RuntimeError ('the temporary directory must be located in an existing directory')
        q.system.fs.removeDirTree(tempDir, onlyLogWarningOnRemoveError=True)
        q.system.fs.createDir(tempDir)
        pcklist = self._listChangedPackagesAsObjects(earlierFile, laterFile, '+')
        # get all qpackages from the location specified in bundledownload in /opt/qbase3/cfg/qpackages4/sources.cfg
        for pck in pcklist:
        ...
        domainlist = q.system.fs.listDirsInDir(tempDir, recursive=False, dirNameOnly=True)         
        tarpath = q.system.fs.joinPaths(tempDir, tarName) 
        ...
        try:   
            for domain in domainlist:
                ...
                metadatatarfile = q.system.fs.joinPaths(q.qp.getMetaTarPath(domain), metadatabranchfile)
                domaindir = q.system.fs.joinPaths(tempDir, domain)
                if q.system.fs.exists(metadatatarfile):
                    q.system.fs.copyFile(metadatatarfile, domaindir)
                else:
                    failed = True
                ...
                q.system.fs.removeDirTree(domaindir, onlyLogWarningOnRemoveError=True)
        finally:
            tar.close()
            if failed:
                q.system.fs.removeDirTree(tempDir, onlyLogWarningOnRemoveError=True)  # clean up everything            
