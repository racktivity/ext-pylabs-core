...
class ClusterSSHClient():
    ...
    def connect(self,force=False,testonly=False):
        ...
        if self.isconnected==True and force==False:
            return True
        if not q.system.net.pingMachine(self.node.ipaddr,5):
        ...
        q.logger.log( "connect %s" % self.node.ipaddr)
        state="start"
        passwdworking = None
        if self.passwd<>"":
        ...
        for passwd in passwords2try:
            if state=="start" or state=="autherror":
                try:
                    ...
                    #q.logger.log("clustersshclient: try to login with passwd %s on %s" %(passwd,self.node.ipaddr),2)
                    self._sshclient=q.remote.system.connect(self.node.ipaddr,"root",passwd,timeout=10)
                    state="done"
                    q.logger.log("cluster node %s connected over ssh" % self.node.ipaddr,5)
                    passwdworking=passwd
                    
                except q.remote.system.exceptions.RemoteSystemAuthenticationError:#:AuthenticationException,e:
                    state="autherror"
                    q.logger.log("Could not sshlogin into %s with login %s, passwd %s" % (self.node.ipaddr,"root",passwd))
                #except Exception, e:
    ...
    def execute(self,command,dieOnError=True, timeout=None):
        self.connect()
        ...
        returncode,stdout=self._sshclient.process.execute(command, dieOnError, timeout)
        if stderr<>"":
            stdout=stdout+"\nSTDERR:\n"+stderr
        return returncode,stdout
    
    def getSFtpConnection(self):
        q.logger.log("Open SFTP connection to %s" % (self.node.ipaddr))
