...
class Replicator():
    ...
    def loadRecipe(self):
        ...
        for recipeLine in self.recipe:
            if q.system.fs.joinPaths("%s_%s"%(self.baseDir,recipeLine.sync2nodes), recipeLine.pathinqbase) not in self.dirs2monitor.itervalues():
                if self.dirs2monitor.has_key(recipeLine.sync2nodes):
                    self.dirs2monitor[recipeLine.sync2nodes].append(q.system.fs.joinPaths("%s_%s"%(self.baseDir,recipeLine.sync2nodes),recipeLine.pathinqbase))
                else:
                    self.dirs2monitor[recipeLine.sync2nodes]=[q.system.fs.joinPaths("%s_%s"%(self.baseDir,recipeLine.sync2nodes),recipeLine.pathinqbase),]
    
    def _copyDir(self, path, cluster, nodetype):
        for dir in q.system.fs.listDirsInDir(path,True):
            self._copyDir(dir, cluster, nodetype)
        for file in q.system.fs.listFilesInDir(path,True):
            self._copyFile(file, cluster, nodetype)

    def _copyFile(self, path, cluster, type=None):
        if q.codetools.regex.matchMultiple(self.replicationIncludeRegexes, path):
            #print "PATH:%s" % path
            #if event==1:
            ...
            if not type:
                for nodetype,dirs in self.dirs.iteritems():
                    if q.system.fs.getDirName(path).rstrip('/') in dirs:
                        type=nodetype
                        break
            destpath=path.replace("qbase3debug_%s"%type,"qbase3")
            if destpath not in self._createddirs:
                cluster.mkdir(q.system.fs.getDirName(destpath),hostnames=cluster.listnodes(type=type) )
            ...
            for dir in taskletDir:
                ...
                cluster.execute('touch %s'%q.system.fs.joinPaths(q.dirs.baseDir, dir, 'tasklets_updated'), hostnames=cluster.listnodes(type=type))
    ...
    def start(self,clustername="", copyFiles=True):
        ...
        if clustername=="":
            clustername=q.console.askChoice(q.cluster.list(),"Choose cluster")
        answer=q.console.askYesNo("Are you sure you want to replicate to cluster %s" %clustername)
        if not answer:
            print "Exiting replicator . . ."
            return
        if copyFiles:
            copyFiles = q.console.askYesNo("Do you want to synchronize the complete development sandbox to %s now" %clustername)
        ...
        if copyFiles:
            ...
            for nodetype in ['all', 'master']:
                cluster.execute('/usr/bin/rsync -aL %s::%s/%s_%s/* %s'%(localip, modulename, modulename, nodetype, q.dirs.baseDir),hostnames=cluster.listnodes(nodetype))
        ...
        def callback(path,event,data):
            ...
            path="%s/%s" % (data,path)
            path=path.strip()
            if q.system.fs.exists(path) and start and (event==1 or event==2):
        ...
        for nodetype,dirlist in self.dirs.iteritems():
            for item in dirlist:
                ...
                if localcounter==200:  # a new monitor is needed per 200 otherwise can lock
                    ...
                    #time.sleep(2)
                    localcounter=0
                q.logger.log( "%s/%s: add dir %s to monitor %s" % (counter,len(dirlist),item,len(self.monitors)),2)
