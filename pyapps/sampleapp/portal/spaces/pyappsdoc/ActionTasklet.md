...
For each Root Object you have created an [interface](/sampleapp/#/pyappsdoc/Action). The interface file contains the different actions that you can perform on the Root Object.
...
The `params` dictionary lives throughout the whole PyLabs framework. You will see in the [Creating Forms and Wizards](/sampleapp/#/pyappsdoc/FormWizardParctical) section, that the wizards and forms gather the data. This data is passed on to the cloud API call, which on its turn calls this tasklet. 
...
All other properties, which are defined in the [modeling](/sampleapp/#/pyappsdoc/Modeling) phase are empty.
...
It is also required to add the guid of the object to the keyword 'result' in the `params` dictionary. See the [Defining Actions Interface on Root Objects](/sampleapp/#/pyappsdoc/action) chapter for more information.
...
##Creating Lists
In the [OSIS Views](/sampleapp/#/pyappsdoc/osisviews) chapter, you have learned how you can create different views on a Root Object. These views can now be used to create a list.
