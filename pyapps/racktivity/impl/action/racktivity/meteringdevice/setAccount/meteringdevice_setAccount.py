...
def main(q, i, p, params, tags):
    ...
    #result = p.api.actor.racktivity.meteringdevice.setConfigurationParameter(meteringdeviceguid, master.meteringdevicetype, masteripaddress, deviceapiport, meteringdevice.id,
    #                                                                  "AdminLoginAndPassword", login, master.accounts[0].login, master.accounts[0].password)
    result = p.api.actor.racktivity.meteringdevice.setAccount(master.meteringdevicetype, master.network.ipaddress, master.network.port,
                                                       master.accounts[0].login, master.accounts[0].password,
                                                       login, password, usertype)

    if not result['result']:
        raise ValueError("Failed to set login %s"% master.guid)
    ...
    if not result['result']:
        raise ValueError("Failed to set username/password for meteringdevice %s" % master.guid, messageprivate='', typeid='RACTKVITIY-MON-GENERIC-0077', tags='', escalate=False)
